...
不设置 `RUN_QT_WEBENGINE_TESTS` 时部分测试会跳过。测试后打开新的 PowerShell，先运行 `.\.venv-packaging\Scripts\python.exe -m tools.collect_licenses licenses --qt-docs` 获取匹配版本的许可材料（需要网络），核对后再运行 `packaging\build.ps1`。公开测试优先发布由 `packaging\installer.iss` 生成的标准安装包，并保留便携 ZIP 作为备用；发布前必须使用最新病毒库扫描。`packaging\build-onefile.ps1` 仅保留用于本地打包实验，不建议公开分发未签名的单文件自解压版本。构建会清理项目内对应生成目录，不要在 dist 中保存个人文件。
