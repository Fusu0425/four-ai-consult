...
不设置 `RUN_QT_WEBENGINE_TESTS` 时部分测试会跳过。测试后打开新的 PowerShell，先运行 `.\.venv-packaging\Scripts\python.exe -m tools.collect_licenses licenses --qt-docs` 获取匹配版本的许可材料（需要网络），核对后再运行 `packaging\build.ps1`。可另外运行 `packaging\build-onefile.ps1` 生成无需解压的单文件测试版；它启动较慢，不能替代便携 ZIP 的兼容性测试。构建会清理项目内对应生成目录，不要在 dist 中保存个人文件。
